...
export default function ProductsServicesCategoriesDashboard() {
	const { productsAndServices, setSelectedProduct } = useStates();
					...
					<div className="space-y-5 sm:space-y-6">
						<div className="lg:col-span-9">
							<div className="flex items-center justify-between flex-wrap gap-4">
								...
								<div className="flex md:max-w-xs w-full order-3">
								...
								<select
									id="sort"
									name="sort"
									className="block w-fit py-2 pl-1 pr-1 text-base bg-transparent border-gray-300 border-none rounded-lg focus:outline-none focus:ring-indigo-600 focus:border-indigo-600 sm:text-sm"
								>
													...
													{filteredProductsAndServices?.map((item) => (
														...
														>
															...
															<td className="hidden px-4 py-4 text-sm font-bold text-gray-900 sm:px-6 lg:table-cell whitespace-nowrap">
																{item?.serviceCycle
																	? `${item?.serviceCycle?.unit}   ${item?.serviceCycle?.duration} `
																	: "N/A"}
															</td>
															<td className="px-4 py-4 text-sm font-medium text-right text-gray-900  whitespace-nowrap">
																<button
																	onClick={(e) => {
																		e.stopPropagation();
																		const selected =
																			productsAndServices?.offerings?.find(
																				(i) => i.id === item.id
																			);
																		setSelectedProduct(selected);
																		navigate(
																			`/products-services/${item.id}/edit`
																		);
																	}}
