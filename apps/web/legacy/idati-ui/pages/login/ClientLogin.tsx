	...
	async function loginHandler() {
		...
		try {
			...
			const res = await axios.post(
				`${BASE_URL}/api/v2/auth/customer/login`,
				body,
				config
			);
			...
			if (res) {
				if (res?.data?.data) {
					...
					const returnedUser = {
						...
						accessToken: userData?.authToken?.token,
					...
					setAccessToken(userData?.authToken?.token);
					const dateExpire = Date.now() + 23 * 60 * 60 * 1000;

					localStorage.setItem("accessToken", userData?.authToken?.token);
					localStorage.setItem("expireAt", dateExpire.toString());

					const businessInfo = res?.data?.data?.host_business;
			...
			<div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
				<div className="max-w-2xl mx-auto text-center">
					...
					<p className="max-w-xl mx-auto mt-4 text-base leading-relaxed text-gray-600">
						Kindly enter your credentials to login to your account
					</p>
				</div>
