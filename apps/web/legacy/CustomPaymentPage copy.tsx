...
import { getRequest, postMulti, postRequest } from "../../utils/fetch-function";
...
export default function CustomPaymentPage() {
	...
	return (
		<section className="py-12 bg-white ">
			<LoadingModal />
			<div className="bg-white p-10 max-w-4xl mt-2 mx-auto  rounded-md text-sm text-gray-800 border rounded-lg shadow-lg">
				{/* Header */}
				...
				<div className="flex justify-between border-b-4 border-gray-600 py-3 mb-6 font-medium">
					<div>
						<p>
							...
							<span className="font-semibold">
								{invoiceToPay?.businessId?.name}
							</span>
						</p>
						<p>
							Due Date:{" "}
							<span className="font-semibold">
								{formatDate(invoiceToPay?.due)}
							</span>
						</p>
					</div>
					<div className="text-right">
						...
						<p className="text-gray-900 font-bold text-lg">
							Amount Due: {Number(invoiceToPay?.order?.amount).toFixed(3)}
						</p>
					</div>
				</div>
						...
						{invoiceToPay?.order?.items?.map((item, index) => (
							<tr key={index} className="border-b border-gray-300 text-sm">
								...
								<td className="py-2 text-right">
									{item.amount?.toFixed(3)} {item.currency}
								</td>
						...
						<table className="w-full text-sm mb-4">
							<tbody>
								...
								<tr>
									...
									<td className="text-right">
										- {invoiceToPay?.order?.discount?.toFixed(3) || "0.00"}{" "}
										{invoiceToPay?.order.currency}
									</td>
								</tr>
								<tr>
									...
									<td className="text-right">
										+ {invoiceToPay?.order?.tax?.toFixed(3) || "0.00"}{" "}
										{invoiceToPay?.order.currency}
									</td>
								</tr>
